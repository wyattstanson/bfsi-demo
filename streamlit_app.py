...
st.set_page_config(page_title='NBA Studio, BFSI personalization', page_icon='📊', layout='wide', initial_sidebar_state='collapsed')
_AVA_SVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40"><circle cx="20" cy="20" r="20" fill="#3B4D8F"/><path d="M13 22a7 7 0 0014 0" stroke="#fff" stroke-width="2.2" fill="none" stroke-linecap="round"/><circle cx="15" cy="17" r="1.3" fill="#fff"/><circle cx="25" cy="17" r="1.3" fill="#fff"/></svg>'
...
def _personas(limit: int=18):
    rows = db.fetchall('SELECT party_id, domain, journey_stage, risk_band FROM party LIMIT ?', (limit,))
    return {f'{p}, {d}/{s}, {r}': p for p, d, s, r in rows}
_ICON = {'shield': '<path d="M12 3l7 3v5c0 4-3 7-7 8-4-1-7-4-7-8V6l7-3z"/>', 'bank': '<path d="M3 10l9-5 9 5"/><path d="M5 10v8M9 10v8M15 10v8M19 10v8M3 21h18"/>', 'card': '<rect x="3" y="6" width="18" height="12" rx="2"/><path d="M3 10h18"/>', 'chart': '<path d="M4 19V5M4 19h16M8 15l3-4 3 2 4-6"/>', 'handshake': '<path d="M8 12l3 3 5-5"/><path d="M3 10l4-4 5 3 5-3 4 4"/>', 'headset': '<path d="M4 13v-1a8 8 0 0116 0v1"/><rect x="3" y="13" width="4" height="6" rx="1"/><rect x="17" y="13" width="4" height="6" rx="1"/>', 'spark': '<path d="M12 4v16M4 12h16"/>'}

def _icon_svg(aid: str) -> str:
    key = 'shield' if 'fraud' in aid else 'bank' if 'loan' in aid else 'card' if 'card' in aid else 'chart' if 'wealth' in aid else 'handshake' if 'retain' in aid else 'headset' if 'support' in aid else 'spark'
...
with left:
    ...
    event_type = st.selectbox('Live context signal', ['session_start', 'large_transfer_attempt', 'product_page_view', 'card_declined'])
    if st.button('Recommend next-best-action', type='primary', use_container_width=True):
        pid = personas[persona_label]
        event = {'event_type': event_type, 'amount': 4000 if event_type == 'large_transfer_attempt' else 0}
    ...
    if 'last_decision' in st.session_state:
        ...
        if rej:
            st.markdown(f'<div class="seclab">Ruled out</div><div style="font-size:12px;color:#6B7686">{rej}</div>', unsafe_allow_html=True)
...
with right:
    ...
    for m in st.session_state.chat:
        with st.chat_message(m['role'], avatar=AVA if m['role'] == 'assistant' else None):
            if m.get('greet'):
                st.write('Hi, I am **Ava**. I can act on a customer request end to end, and I bring in a human when the stakes are high. Use a quick action or type below.')
            elif m['role'] == 'user':
                st.write(m['text'])
            else:
                for ev in m.get('steps', []):
                    esc = ev['node'] == 'escalate'
                    st.markdown(f"""<div class="stepline">{('!' if esc else '•')} <b>{ev['node']}</b>, {ev['detail']}</div>""", unsafe_allow_html=True)
                f = m.get('final')
                if f:
                    if f['escalated']:
                        st.markdown('<div class="escb">Handed to a human specialist. ' + f['answer'].replace('Escalated to human review: ', '') + '</div>', unsafe_allow_html=True)
                    else:
    ...
    with st.form('ava_composer', clear_on_submit=True):
        msg = st.text_input('msg', label_visibility='collapsed', placeholder='Ask Ava to handle something')
        if st.form_submit_button('Send', type='primary', use_container_width=True) and msg.strip():
            _submit(msg.strip())
            st.rerun()
with st.sidebar:
    st.header('Governance')
    st.json({'drift': governance.drift_snapshot(), 'fairness': fairness.snapshot(), 'human_queue': (db.fetchone('SELECT COUNT(*) FROM human_queue') or [0])[0]})
    st.caption('Structural reference demo, synthetic data and illustrative scores. The architecture, latency and audit trail are the point.')
