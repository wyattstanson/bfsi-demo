...
def run(n: int=500) -> int:
    ...
    body = {'party_id': None, 'channel': 'web', 'event': {'event_type': 'large_transfer_attempt', 'amount': 4000}}
    if base:
        import httpx
    ...
    for label, lat in (('server decision', server_lat), ('client wall', wall_lat)):
        print(f'  {label:16s}  p50={_percentile(lat, 50):6.2f}ms  p95={_percentile(lat, 95):6.2f}ms  p99={_percentile(lat, 99):6.2f}ms')
