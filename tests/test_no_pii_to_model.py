"""Acceptance: payloads that reach the model/LLM carry ids, never PII."""
...
def test_guardrail_rejects_pii():
    tools.assert_no_pii({"party_id": "P1", "profile": {"risk_band": "low"}})  # ok
...
def test_llm_payload_contains_no_pii(client, party_id, monkeypatch):
    # Ground-truth PII from the ledger that must NOT leak to the model.
    name, email, ssn = db.fetchone(
        "SELECT name, email, ssn FROM party WHERE party_id = ?", (party_id,)
    )
    ...
    def spy(query, docs):
        ...
        captured["docs"] = docs
        return {"answer": "ok", "citations": [{"rank": 1, "snippet": docs[0] if docs else ""}],
                "grounded": True}
