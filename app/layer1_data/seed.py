...
def _make_party(i: int) -> tuple:
    ...
    region = random.choice(list(config.REGIONS))
    currency = config.REGIONS[region]
    age = random.randint(21, 78)
    income = round(random.lognormvariate(11, 0.5), 2)
    balance = round(max(0.0, random.lognormvariate(9, 1.2)), 2)
    tenure = random.randint(0, 240)
    return (f'P{i:06d}', domain, stage, region, currency, fake.name(), fake.email(), fake.ssn(), age, income, balance, tenure, random.choice(_RISK), random.choice(config.FAIRNESS_GROUPS), f'consent-{i:06d}', 1 if random.random() < 0.7 else 0)
...
def _make_txns(party_id: str, n: int) -> list[tuple]:
    ...
    for _ in range(n):
        ...
        rows.append((party_id, now - random.uniform(0, 90 * 86400), amount, random.choice(['5411', '6011', '4829', '5812', '6300']), random.choice(config.CHANNELS), flagged))
    return rows
...
def seed(n_parties: int | None=None) -> dict:
    ...
    db.executemany('INSERT INTO party\n           (party_id, domain, journey_stage, region, currency, name, email, ssn,\n            age, income, balance, tenure_months, risk_band, fairness_group,\n            consent_ref, consent_marketing)\n           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', parties)
    txns: list[tuple] = []
    for p in parties:
        txns.extend(_make_txns(p[0], random.randint(3, 40)))
    db.executemany('INSERT INTO txn (party_id, ts, amount, mcc, channel, is_flagged) VALUES (?,?,?,?,?,?)', txns)
