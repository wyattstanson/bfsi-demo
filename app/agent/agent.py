...
def _parse_goal(goal: str) -> dict:
    g = goal.lower()
    amount_match = re.search('(\\d[\\d,]*\\.?\\d*)', g)
...
def perceive(state: dict) -> str:
    ...
    state['model_payload'] = {'party_id': state['party_id'], 'intent': state['parsed']['intent'], 'profile': profile, 'policy': docs}
    tools.assert_no_pii(state['model_payload'])
...
def reason(state: dict) -> str:
    ...
    need = {'loan': ['get_bureau_score', 'check_sanctions'], 'transfer': ['check_sanctions'], 'dispute': ['check_sanctions'], 'info': []}[intent]
...
def decide(state: dict) -> str:
    ...
    if intent in ('loan', 'transfer'):
        ...
        posted = tools.call('post_ledger_entry', party_id=state['party_id'], amount=amount or 0.0, memo=memo)
...
def _escalate(state: dict, reason: str, confidence: float) -> str:
    context = {'profile': state['gathered'].get('profile'), 'tools': state['gathered'].get('tools', {}), 'policy_hits': state['gathered'].get('policy_hits', []), 'parsed': state['parsed']}
    tools.assert_no_pii(context)
    db.execute('INSERT INTO human_queue (party_id, goal, reason, context, ts, resolved) VALUES (?,?,?,?,?,?)', (state['party_id'], state['goal'], reason, db.as_json(context), time.time(), 0))
    ...
    return 'end'
_NODES = {'perceive': perceive, 'reason': reason, 'act': act, 'observe': observe, 'decide': decide}

def _initial_state(party_id: str, goal: str) -> dict:
    return {'party_id': party_id, 'goal': goal, 'parsed': _parse_goal(goal), 'gathered': {}, 'steps': [], 'confidence': 1.0, 'escalated': False, 'done': False, 'answer': None, 'engine': 'langgraph' if _HAS_LANGGRAPH else 'hand-rolled'}
...
def run_stream(party_id: str, goal: str):
    ...
    while node != 'end' and guard < MAX_STEPS * 3:
        ...
        while steps_emitted < len(state['steps']):
            ...
            steps_emitted += 1
    yield {'type': 'final', 'answer': state['answer'], 'escalated': state['escalated'], 'confidence': state['confidence'], 'engine': state['engine'], 'citations': state.get('citations', [])}
...
if __name__ == '__main__':
    ...
    for g in ['I want a pre-approved loan of 50000', 'Dispute an unauthorized charge of 15000', 'Give me a financial tip']:
        r = run(pid, g)
        print(f"\nGOAL: {g}\n  -> {r['answer']}  (escalated={r['escalated']}, conf={r['confidence']}, engine={r['engine']}, steps={len(r['steps'])})")
