...
def get_customer_profile(party_id: str) -> dict:
    row = db.fetchone('SELECT domain, journey_stage, risk_band, tenure_months, consent_marketing FROM party WHERE party_id = ?', (party_id,))
    if not row:
        return {'party_id': party_id, 'found': False}
    return {'party_id': party_id, 'found': True, 'domain': row[0], 'journey_stage': row[1], 'risk_band': row[2], 'tenure_months': row[3], 'has_marketing_consent': bool(row[4])}
...
def get_bureau_score(party_id: str) -> dict:
    ...
    return {'party_id': party_id, 'bureau_score': score, 'band': 'prime' if score >= 700 else 'subprime'}
...
def check_sanctions(party_id: str) -> dict:
    ...
    hit = _deterministic_int(party_id, 'ofac') % 100 < 4
...
def post_ledger_entry(party_id: str, amount: float, memo: str) -> dict:
    ...
    return {'party_id': party_id, 'posted': True, 'amount': amount, 'ref': ref, 'ts': time.time()}
