"""Layer 3 — Models: train + version all scorers on the seeded data.

Builds the training matrix from the offline feature table (Layer 2), synthesizes
labels/treatment (no real outcomes exist in a demo), fits every model, and saves
versioned artifacts.  Run once after seeding + materializing features.
"""
...
def _matrix() -> tuple[list[str], np.ndarray]:
    """Pivot the offline feature table into an ordered matrix."""
    ...
    ids = sorted(by_party)
    X = np.array(
        [[by_party[p].get(f, 0.0) for f in feature_store.FEATURES] for p in ids],
        dtype=float,
    )
    return ids, X
...
def _labels(X: np.ndarray, rng: np.random.Generator):
    # Column indices follow feature_store.FEATURES order.
    txn_count, _sum, _avg, flagged, balance, tenure, income, age = X.T

    # Cross-sell propensity: wealthier, active parties are likelier to convert.
    z_prop = 1e-5 * balance + 2e-5 * income + 0.03 * txn_count - 4.0
    y_prop = (rng.random(len(X)) < _sigmoid(z_prop)).astype(int)

    # Churn: short tenure + flagged activity + low engagement raises risk.
    z_churn = 3.0 * flagged - 0.01 * tenure - 0.02 * txn_count + 0.5
    y_churn = (rng.random(len(X)) < _sigmoid(z_churn)).astype(int)

    # Uplift: random treatment; effect is largest for mid-tenure mid-income.
    T = (rng.random(len(X)) < 0.5).astype(int)
    effect = 0.25 * np.exp(-((tenure - 60) ** 2) / (2 * 40**2))
    z_base = 1e-5 * income - 1.0
...
def train() -> dict:
    ...
    return {
        ...
        "n": len(ids),
        "algos": {
            "propensity": propensity.algo,
            "churn": churn.algo,
            "uplift": uplift.algo,
            "fraud": fraud.algo,
        },
        "positives": {
            "propensity": int(y_prop.sum()),
            "churn": int(y_churn.sum()),
        },
    }
