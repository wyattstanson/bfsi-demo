"""Layer 3 — Models: uplift / heterogeneous treatment effect.

Answers "who should we target", not "who will convert".  Uses EconML's X-learner
when importable (the named library); otherwise a two-model T-learner built from
scikit-learn: uplift = P(convert | treated) - P(convert | control).

# PROD: swap for EconML on SageMaker, or an in-house causal ML service.
"""
...
try:
    from econml.metalearners import XLearner  # type: ignore
...
class UpliftModel:
    """Estimates individual treatment effect (uplift) in [-1, 1]."""
    ...
    def __init__(self, version: str) -> None:
        ...
        self._x = None  # econml learner when available
